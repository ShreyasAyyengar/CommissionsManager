        ...
        if (event.getComponentId().equalsIgnoreCase("menu:commissions")) {
            ...
            clientInfo.getCommissions().stream().filter(commission -> commission.getPluginName().equals(pluginName)).findFirst().ifPresent(commission -> {
                ...
                List<Button> buttons = List.of(
                        Button.primary("commission-info." + pluginName, "Commission Information").withEmoji(Emoji.fromMarkdown("\uD83D\uDCDD")),
                        Button.secondary("invoice-info." + pluginName, "Invoice Management").withEmoji(Emoji.fromMarkdown("\uD83D\uDCB2"))
                );
            ...
            for (ClientCommission commission : clientInfo.getCommissions()) {
                ...
                commission.getInvoices().stream().filter(invoice -> invoice.getID().equalsIgnoreCase(invoiceID)).findFirst().ifPresent(invoice -> {
                    ...
                    List<Button> invoiceButtons = List.of(
                            Button.secondary("invoice-management." + invoice.getID() + ".nudge", "Nudge Payment").withEmoji(Emoji.fromMarkdown("\uD83D\uDDE3")),
                            Button.secondary("invoice-management." + invoice.getID() + ".view-info", "View Invoice Information").withEmoji(Emoji.fromMarkdown("\uD83E\uDDFE")),
                            Button.secondary("invoice-management." + invoice.getID() + ".file-holding", "Add files to Holding").withEmoji(Emoji.fromMarkdown("\uD83D\uDCC1")),
                            Button.danger("invoice-management." + invoice.getID() + ".cancel", "Cancel Invoice").withEmoji(Emoji.fromMarkdown("⛔"))
                    );
